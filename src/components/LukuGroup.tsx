...
export function LukuGroup({ luku, paaluokkaNumero, cuts, onSetCut, onRemoveCut }: Props) {
  ...
  const lukuCutTotal = luku.momentit.reduce((sum, m) => {
    ...
    return sum + (entry ? getSavingsForEntry(m.id, entry) : 0)
  }, 0)

  const hasCuts = lukuCutTotal > 0

  return (
    <div className={`border-l-2 ml-2 transition-colors ${hasCuts ? 'border-rose-300' : 'border-slate-200'}`}>
      <button
        onClick={() => setExpanded(!expanded)}
        className="w-full flex items-center justify-between px-3 py-2 text-left hover:bg-slate-50/50 transition-colors rounded-r-lg"
      >
        <div className="flex items-center gap-2 min-w-0">
          <svg className={`w-3 h-3 shrink-0 text-slate-400 transition-transform duration-200 ${expanded ? 'rotate-90' : ''}`} fill="currentColor" viewBox="0 0 20 20">
            <path d="M6 4l8 6-8 6V4z" />
          </svg>
          <span className="text-sm font-medium text-slate-600 truncate">
            <span className="text-indigo-400/60 font-mono text-xs mr-1">{paaluokkaNumero}.{luku.numero}</span>
            {luku.nimi}
          </span>
        </div>
        <div className="flex items-center gap-3 shrink-0 ml-2">
          ...
          <span className="text-xs text-slate-400 font-mono">{formatEur(luku.total)}</span>
        </div>
      </button>
      {expanded && (
